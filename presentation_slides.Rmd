...
Cadre typique : le premier tableau contient plutôt des variables réponses ; le deuxième tableau contient plutôt des variables explicatives. <br>

--

Exemple : le premier tableau contient, pour différentes stations de mesures, les données d'abondance de 10 espèces de poissons ; le deuxième tableau contient, pour les mêmes stations de mesures, des variables environnementales. <br>
...
Pour cela, on calcule le coefficient RV entre les 2 tableaux (coefficient de corrélation). Puis on permute les lignes d'un des 2 tableaux, et on recalcule le coefficient RV en prenant en compte ce 'nouveau tableau'. On itère ce processus un grand nombre de fois (au moins 900 fois).

--

Attention : le tableau sur lequel on effectue les permutations de ligne ne doit pas être celui qui a fourni les pondérations utilisées en analyse factorielle.

--

.center[
```{r echo=FALSE, out.height= "30%", out.extra=FALSE}
...
On crée un jeu de données qui contient, pour 16 stations de mesure différentes, les données d'abondance de différents poissons. On a donc 16 individus et 10 variables.
...
On constate que pour chaque station, certains poissons sont présents et d'autres sont absents. Les stations semblent être réparties sur un gradient (mais de quelle variable ?) qui conditionne la présence et l'absence de certaines espèces.
...
On crée un jeu de données qui contient, pour les mêmes 16 stations de mesure, la température à chaque heure de la journée. On a donc 16 individus et 24 variables.

--

La représentation graphique suivante permet de bien observer les différences de température entre chaque station (ainsi que la composante circadienne et l'aléa généré, le 'bruit') :
...
```{r out.height="35%", out.extra=FALSE}
afc <- dudi.coa(data.vv,scannf = F)
scatter(afc)
```
...
```{r eval=TRUE, out.extra=FALSE, out.height="35%"}
...
Le premier axe de l'ACP explique plus de 99% de la variabilité : on peut supposer qu'il s'agit du gradient de température entre les différentes stations.
...
```{r eval=TRUE, out.extra=FALSE, out.height="35%"}
plot(Tstations, acp$li[,1], ylab = "F1", xlab = "Température", pch = 19)
```

--

La première valeur propre de l'ACP, F1, correspond donc bien au gradient de température.
...
Regardons les premières valeurs propres :

```{r eval=TRUE}
cia$eig[1]/sum(cia$eig) ; cia$eig[2]/sum(cia$eig) ; cia$eig[3]/sum(cia$eig)
```

Le premier facteur extrait 99.3% de la variabilité : c'est sans doute le gradient thermique.
...
```{r warning=FALSE, out.height="35%", out.extra=FALSE}
plot(ciatest)
```
