...
```{r warning=FALSE, out.height="40%", out.width="40%"}
...
```
On visualise bien l'effet Guttman (les données ont une forme de parabole). <br>
Cet effet témoigne de liaisons fortes entre les variables. (A VERIFIER)
]
.panel[.panel-name[rearrange]
Cela permet de ré-ordonner le tableau pour mettre en évidence le gradient
```{r warning=F, out.height="40%", out.width="40%"}
...
On a les températures pour les 24 heures de la journée et ce pour chaque station.
...
On observe un gradient des stations froides aux stations chaudes et une composante circadienne.<br>
.center[
```{r echo=FALSE, eval=TRUE}
plot(0, type = "n", xlim = c(1,24), ylim = range(mesureT), xlab = "Heures",ylab = "Temperature",
     las = 1, main = "Température des stations") # Cadre vide
...
Le gradient est difficile à observer ici dû à l'aléa de température utilisé
.center[
```{r echo=FALSE, eval=TRUE}
table.value(mesureT)
```
...
On fait l'Analyse en Composante Principales du second tableau
...
.panel[.panel-name[ACP plot]
On observe que le premier axe participe très largement à la construction de l'ACP... <br> 
.center[
```{r echo=FALSE, eval=TRUE}
scatter(acp)
```
...
.panel[.panel-name[1st axis plot]
... car il s'agit du gradient de température qui structurait notre jeu de données initialement! <br>
.center[
```{r echo=FALSE, eval=TRUE}
plot(Tstations, acp$li[,1], ylab = "F1", xlab = "Température",
     main = "Le premier facteur de l'ACP est le gradient thermique", pch = 19)
...
## Vérifications 
.panelset[
.panel[.panel-name[Vérification 1]
...
.panel[.panel-name[Vérification 2]
On vérifie que les individus sont bien dans le même ordre dans les deux tableaux. <br>
...
.panel[.panel-name[Vérification 3]
On vérifie que la même pondération a été utilisée pour les deux analyses.
```{r}
...
---
# Réalisation de la co-inertie
Tout est bon, on peut utiliser l'analyse de co-inertie pour coupler les deux tableaux. <br>

.panelset[
.panel[.panel-name[Co-inertie]
```{r}
cia <- coinertia(dudiX = pca, dudiY = coa, scannf = FALSE, nf = 2)
```
]
.panel[.panel-name[Valeurs propres]
On regarde les premières valeurs propres 
```{r}
cia$eig[1]/sum(cia$eig)
cia$eig[2]/sum(cia$eig)
cia$eig[3]/sum(cia$eig)
```
